...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSLog(@"getting from Core Data...");
    
    /* FETCHING */
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"AZFriend"];
    
    NSString *name = @"Alexey";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"name == %@", name];
    [request setPredicate:pred];
    NSError *err;
    NSAsynchronousFetchResult *res = [self.persistentContainer.viewContext executeRequest:request error:&err];
    NSArray *arr = res.finalResult;
    if (arr.count <= 0){
        return YES;
    }
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"_1_CoreDataCustom"];
